...
{
    const char *graph_fic_nom = "gr1.txt";
    FILE *fp;
...
void remplissage_mat_surv(Grille *G, int **matrice_cible, int **matrice_surveillant)
{
    for (int i = 0; i < G->nb_lignes; i++)
    {
        for (int j = 0; j < G->nb_colonnes; j++)
        {
            if (matrice_surveillant[i][j] == 1)
            { // on peut prendre ce surveillant
...
int calcul_nbre_cible_par_lignes(int ligne, int colonne, int **matrice_cible, int **matrice_surveillant, int nbre_colonne, Grille *G)
{
    ...
    while (i < nbre_colonne && G->matrice_adjacence[ligne][i] != -1)
    { // pas de d'obstacle
        if (G->matrice_adjacence[ligne][i] == 1 && matrice_cible[ligne][i] == 0)
        { // une cible non couverte
            res++;
        }
        i++;
    }
    ...
    while (i >= 0 && G->matrice_adjacence[ligne][i] != -1)
    { // pas de d'obstacle
        if (G->matrice_adjacence[ligne][i] == 1 && matrice_cible[ligne][i] == 0)
        { // une cible non couverte
            res++;
...
int calcul_nbre_cible_par_colonnes(int colonne, int ligne, int **matrice_cible, int **matrice_surveillant, int nbre_ligne, Grille *G)
{
    ...
    while (i < nbre_ligne && G->matrice_adjacence[i][colonne] != -1)
    { // pas de d'obstacle
        if (G->matrice_adjacence[i][colonne] == 1 && matrice_cible[i][colonne] == 0)
        { // une cible non couverte
            res++;
        }
        i++;
    ...
    while (i >= 0 && G->matrice_adjacence[i][colonne] != -1)
    { // pas de d'obstacle
        if (G->matrice_adjacence[i][colonne] == 1 && matrice_cible[i][colonne] == 0)
        { // une cible non couverte
            res++;
...
{
    int i = colonne + 1;
    while (i < G->nb_colonnes && G->matrice_adjacence[ligne][i] != -1)
    { // pas de d'obstacle
        if (G->matrice_adjacence[ligne][i] == 1)
        {                                // une cible trouvée
...
void couvrir_par_colonnes(int colonne, int ligne, int **matrice_cible, int **matrice_surveillant, int nbre_ligne, Grille *G)
{
    ...
    while (i < G->nb_lignes && G->matrice_adjacence[i][colonne] != -1)
    { // pas d'obstacle
        if (G->matrice_adjacence[i][colonne] == 1)
        {                                  // une cible non couverte
